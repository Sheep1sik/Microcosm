...
extension UniverseSceneRenderer {
    ...
    func setupDustField() {
        let worldSize = UniverseSceneFeature.CameraState.worldSize
        ...
        for i in 0..<600 {
            ...
            sprite.position = CGPoint(
                x: CGFloat.random(in: 0...worldSize.width),
            ...
            sprite.setValue(
                SKAttributeValue(vectorFloat4: vector_float4(Float(cr), Float(cg), Float(cb), 1)),
                forAttribute: "a_color"
            )

            if i % 3 == 0 {
                sprite.run(.repeatForever(.sequence([
                    .fadeAlpha(to: CGFloat.random(in: 0.03...0.1), duration: Double.random(in: 3...6)),
                    .fadeAlpha(to: CGFloat.random(in: 0.12...0.3), duration: Double.random(in: 3...6)),
                ])))
            }
            addChild(sprite)
    ...
    func setupNebulae() {
        ...
        for d in data {
            ...
            for _ in 0..<layerCount {
                ...
                sprite.blendMode = .add
                sprite.run(.repeatForever(.sequence([
                    .moveBy(x: CGFloat.random(in: -8...8), y: CGFloat.random(in: -4...4), duration: 12),
                    .moveBy(x: CGFloat.random(in: -8...8), y: CGFloat.random(in: -4...4), duration: 12),
                ])))
                addChild(sprite)
            }
